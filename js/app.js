 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        fetch("https://restcountries.com/v3.1/all").then((response => response.json())).then((data => {
            document.querySelectorAll(".form-list.country").forEach((countryList => {
                let output = "";
                data.forEach((country => {
                    output += `\n\t\t\t\t\t<li class="form-item" data-value="${country.cca2}" data-code="${country.idd.root}${country.idd.suffixes ? country.idd.suffixes[0] : ""}" data-flag="${country.flags.svg}">\n\t\t\t\t\t\t <img src="${country.flags.svg}" alt="${country.name.common} flag">\n\t\t\t\t\t\t <div>${country.name.common} (+${country.idd.root}${country.idd.suffixes ? country.idd.suffixes[0] : ""})</div>\n\t\t\t\t\t</li>`;
                }));
                countryList.innerHTML = output;
                countryList.querySelectorAll(".form-item").forEach((item => {
                    item.addEventListener("click", (function() {
                        const formContainer = this.closest(".form-container");
                        if (formContainer) {
                            const flagIcon = formContainer.querySelector(".flag-icon");
                            const countryCode = formContainer.querySelector(".country-code");
                            if (flagIcon && countryCode) {
                                flagIcon.src = this.dataset.flag;
                                countryCode.innerText = `+${this.dataset.code}`;
                                formContainer.querySelector(".form-list").classList.remove("open");
                                formContainer.querySelector('input[name="country"]').value = this.dataset.value;
                            }
                        }
                    }));
                }));
